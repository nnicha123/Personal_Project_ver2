...
const addOrder = async (req, res) => {
  ...
  const newOrder = await db.order.create({ status, title, price, menu_pic, user_id: req.params.user_id, menu_id: req.params.menu_id })
  res.status(201).send(newOrder)
}
const updateOrder = async (req, res) => {
  const { status, title, price, menu_pic } = req.body
  const targetId = req.params.menu_id
  const foundTarget = await db.order.findOne({ where: { menu_id: targetId } })
  if (!foundTarget) {
    res.status(404).send({ message: 'Target not found' })
  }
  else {
    await foundTarget.update({ status, title, price, menu_pic })
    res.status(200).send({ message: 'Successfully updated' })
  }
}
const deleteOrder = async (req, res) => {
  const targetId = req.params.menu_id
  const foundTarget = await db.order.findOne({ where: { menu_id: targetId, user_id: req.user.id } })
  if (!foundTarget) {
    res.status(404).send({ message: 'Target not found' })
  }
  else {
